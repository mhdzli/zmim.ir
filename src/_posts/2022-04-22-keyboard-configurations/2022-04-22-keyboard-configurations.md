...
گر چه ویرایش فایل‌ها در مسیر ‪`/usr/share/X11/xkb/symbols`‬ امکان شخصی سازی `keymap` را به شما می‌دهد اما از نسخه `libxkbcommon 0.10.0` امکان ایجاد فایل‌های تنظیمات شخصی در ‪‪`$XDG_CONFIG_HOME/xkb`‬‬ و در پوشه‌های ‪‪`$XDG_CONFIG_HOME/xkb/rules/`‬‬ و ‪‪`$XDG_CONFIG_HOME/xkb/symbols`‬‬ فراهم شده است و فایل‌های موجود در این مسیر پیش از فایل‌های سیستم فراخوانی می‌شوند.

به عنوان نمونه کافیست برای افزودن یک آپشن برای جایگزینی کلید `PtrSc` با کلید `Menu` کافیست فایل‌های ‪‪`$XDG_CONFIG_HOME/xkb/rules/envdev`‬‬ و ‪‪`$XDG_CONFIG_HOME/xkb/symbols/custom`‬‬ را با به صورت زیر ایجاد و تنظیم کنیم:
...
و بعد این گزینه را در سیستم فعال/غیرفعال کنیم. برای محیط گنوم:
...
همانطور که گفتم این تنظیمات پیش از مسیرهای اصلی سیستم فراخوانی و اعمال می‌شوند. برای اطلاعات بیشتر می‌توانید [این صفحه](https://who-t.blogspot.com/2020/02/user-specific-xkb-configuration-part-1.html?m=1){:target="_blank"}{:rel="noopener noreferrer"} را بخوانید.
